...
## [5.21.23] - 2026-05-17 - ★TTSPanel ランタイムエラー修正(v5.21.13 残骸 3 箇所目)★

**問題:**
- TTSPanel(音声パネル)を開くと「showAllScripts is not defined」エラー
- 試行・再起動でも改善せず

**原因:**
v5.21.13 の版数ラベル除去スクリプトが、★useState 宣言行を 3 つ★ コメントアウトしたまま残していた。**参照側のコードは生きている**ため、参照時に ReferenceError 発生。

**被害箇所:**
- 行 33: `// const [showAllScripts, setShowAllScripts] = useState(false);` ← 4 箇所で参照
- 行 38: `// const [diagnostic, setDiagnostic] = useState(null);` ← 6 箇所で参照
- 行 40: `// const [exporting, setExporting] = useState(false);` ← 7 箇所で参照

**修正:**
3 つの useState 宣言を全て復活(コメント解除)。

**網羅検査の強化(再々確認):**
過去 v5.21.20-22 では「構文エラー」のみを検査していて、★ランタイムエラー(参照ありで宣言なし)★ は見逃していた。今回は Python スクリプトで:

> 「// で始まる宣言行(const/let/var/function)を全部抽出 → その変数名が別行(非コメント)で参照されている」異常を検出

→ ★これで全パターン網羅★、他に異常なし確認。

**v5.21.13 のスクリプト被害、これで打ち止め(と願う):**
- v5.21.20: config.js 構文エラー(2 行)
- v5.21.21: TTSPanel.jsx 構文エラー(2 箇所)
- v5.21.23: TTSPanel.jsx ランタイムエラー(3 つの useState 宣言)
- 全パターン検査済み → これ以上は無いはず

**再発防止策(私の今後のメモ):**
機械的書き換え後の必須検査:
1. **構文チェック**: `find . -name "*.jsx" | xargs node --check`(JS のみ)
2. **コメント崩れチェック**: コメント行 `{` `(` で終わる + 後続コード + 閉じ括弧
3. **★ランタイム参照チェック★**: コメントアウトされた宣言 + 別行で参照
4. **★ビルドチェック★**: `npm run build` で実機検証(時間あれば)
...
**緊急修正:**
- `src/components/TTSPanel.jsx:104-107` で構文エラー
- 原因: v5.21.13 の版数ラベル除去スクリプトが `// ★vN★ if (...) {` のような行を消す時に、 ★if 文の開き括弧をコメントアウトしたまま、対応する閉じ括弧 `}` を残してしまった★
- 行 104: `// if (p.fallbackCount !== undefined) {` ← 開き括弧コメント化
- 行 106: `}` ← 閉じ括弧は残存 → 構文エラー
- 同様に行 110-113 でも `// setFallbackInfo({` で関数呼び出しの開き行をコメント化、引数行(行 111-113)が宙ぶらりんに

**修正内容:**
- 行 104-106 の if 文を ★復活★(コメント解除)— pregenerate 進捗コールバック内の正常な if ガード
- 行 110-113 の `setFallbackInfo` 呼び出しも ★復活★(コメント解除)— pregenerate 完了後の最終 fallback 情報セット
- 不要な `// ★リセット★` のような版数ラベル風コメントも整理

**網羅再検査:**
v5.21.20 で「config.js の 2 行のみ」と判定したが、TTSPanel.jsx でも同パターンの被害が出ていた → 検査基準が甘かった。
今回はより厳密な検査(Python スクリプトで「コメント行で `{` `(` で終わる + 後続にコードがあり、最終的に `}` `)` で閉じる」異常パターンを全 .jsx/.js で検出)を実施し、★他に異常なし★ を確認。

**再発防止策:**
- 機械的な書き換え後は `npm run build` で必ず検証
- 私自身が将来同じ作業をする時は、コメント行除去前に「対象行が複数行構造の開き行ではないか」を確認

**Vercel ビルド検証推奨:**
- この修正で Vercel ビルドが通るはず
- 通らなかったら、★更に別の被害箇所★ がある可能性 → 報告お願いします

## [5.21.20] - 2026-05-17 - ★ビルドエラー修正 + 企画提案を基本動作に格上げ★

**緊急修正(ビルドエラー):**
- ★`src/lib/config.js` 行 62-63★ で `// // number/...` `// export const TEAM_PRESETS = [` という二重コメントアウトが発生 → Vite ビルド失敗(`ERROR: Expected ";" but found ":"`)
- 原因: v5.21.13 の版数ラベル除去スクリプトの正規表現バグ。`// ★v5.XX★ ...` を除去する処理が、次の行の `export const` を巻き込んでコメントアウトしてしまった
- 修正: `// number/league/team を選択するとプレビューの選手色が自動で切り替わる` + `export const TEAM_PRESETS = [` に復活
- 同様の被害がないか網羅確認: 該当箇所はこの 2 行のみ(全 src/ 検索で `^// export` は 0 件)
- 検証: `node --check config.js` で構文 OK ✅

**再発防止のため、後続作業では:**
- 版数ラベル除去等の機械的書き換えスクリプトを使った後は、必ず `npm run build` で検証
- 私自身が将来同じ作業をする時は、コメント行除去と隣接コード行への影響を厳密に確認

**設計変更: Gem の企画提案を ★基本動作★ に格上げ:**
- ユーザー指摘「★基本企画提案から進めたい★」
- v11 instruction のフロー記述を「ゼロ依頼時のみ企画提案」→「★ほぼ全ケースで企画提案★」に変更
- スキップ条件を ★3 つに限定★:
  1. ユーザーが「即 JSON で」「企画提案不要」「確認なしで」と明示
  2. 同セッション内で既に企画提案 → 合意 → JSON 済みで、部分修正を求める場合
  3. アプリプロンプトに data/script モード明記 ★かつ★ 既存 projectData / scripts が入力に含まれる場合
- 企画提案表に「出力モード(whole / data / script)」項目を追加 — モード混同を防ぐ
- 企画提案表のレイアウト候補に `freetalk` / `verdict_card` を明示

**期待挙動:**
- 「○○の動画作って」 → 企画提案 ✅
- 「○○のデータを修正して動画作って」 → 企画提案 ✅(既存があっても新規動画なら確認)
- 「ぬるっと型で○○の動画」 → 企画提案 ✅(戦略指定があってもタイトル等は確認)
- 「挨拶動画作って」 → 企画提案 ✅
- 「id:5 だけ修正して」 → ★スキップ可★(既出セッション内部分修正)
- 「即 JSON で○○の動画」 → ★スキップ可★(明示指示)

**v11 サイズ変化:** 7,520 → 8,543 字(+1,023、スキップ条件の限定明示分)

**NotebookLM 再アップロード対象:**
- `gemini-custom-gem-instruction-v11.md`(★企画提案フロー強化★)
...
**経緯:**
- ユーザー指摘「指示に更新履歴とか不要って何度も言ってる」
- 既に v5.21.8 で Gem instruction の改修履歴は削除したが、Knowledge / コードコメントには版数ラベル(`★v5.XX★` 等)が大量残存
- 履歴は CHANGELOG にあるべきもの、Knowledge / コードからは ★完全に分離★

**修正:**

**① structure-playbook.md 完全書き直し:**
- 矛盾: 「20-30 個目安、固定ではない」(現状) と「28-30 id × 1.8 秒 = 50-55秒」(古い)が同居 → 統一
- 古いルール: 「同 speaker 連続最大 2 回まで(★旧 4 回から強化★)」 → 「最大 4 回まで」(現状ルール)に修正、履歴ラベル除去
- 削除した節タイトル: `★v5.14.0★` / `★v5.18新★` / `★v5.20.10★` 等の版数表記
- 内容は維持: smartLoop / ミステリー型構成 / テロップ文字量・テンポ / アウトロ設計 / レイアウト切替パターン

**② 全 Knowledge ファイルから版数ラベル網羅除去:**
- `layout-direction.md`: -915 字(★v5.14.0★ 等 15+ 箇所)
- `layout-templates.md`: -939 字(★v5.18.13新★ 等 6+ 箇所)
- `hook-design.md`: -34 字(★v5.20.10★ 2 箇所)

**③ アプリ src/ コード内コメントの版数ラベル除去:**
- 40 ファイル、-3486 字
- `★v5.XX.YY★`「★v5.XX.YY 新規★」「★v5.XX.YY 修正★」「★v5.XX.YY-ZZ★」等のパターン網羅

**最終検証:**
- 全 docs / src 配下に `★v[0-9]` パターン残骸 → ★ゼロ★ ✅
- 全 Knowledge から「構成 Gem」「JSON Gem」「申し送り」言及 → ★ゼロ★ ✅
- Knowledge は ★現在のルールのみ★(履歴は CHANGELOG が一手に担う)

**NotebookLM 再アップロード対象(中身変更)**:
- `structure-playbook.md`(全面書き直し)
- `layout-direction.md`
- `layout-templates.md`
- `hook-design.md`

## [5.21.12] - 2026-05-15 - ★Grok の役割純化(リサーチに徹する)★

**経緯:**
- v5.21.11 までは Grok エージェント側に「構成 Gem への申し送り」「想定 id:1 戦略」セクションが残存していた
- ユーザー指摘「Grok はリサーチに徹してほしい、Gem への申し送りは余計」を受け、★Grok の役割を純化★

**役割分担の純化:**

| エージェント | 担当(正しい設計) |
|---|---|
| **Grok** | リサーチに徹する(数字・指標・取材引用・X 反応の集約・シーズン文脈・差別化ポイント) |
| **★Gem の専管事項★** | 構成判断・動画タイプ選択(縦/横)・レイアウト選定・id:1 戦略・emoji/textSize/scenePreset/se・もえか口調の方向性 |
| アプリ | UI 表示・編集 |

**Grok エージェント側の修正(grok-v2/):**
- ★`grok-agent-master.md`★: 役割定義から「構成 Gem への申し送り」削除。`[FULL_AUTO]` 出力構造の §4「構成 Gem への申し送り」セクションを ★完全削除★(題材判定 / データリサーチ / ファン心理分析の 3 章のみに)
- ★`grok-1-researcher.md` §7★: 「関連トピック(構成 Gem への申し送り)」→「数字の組み合わせメモ」に改称、★動画角度提案を禁止★ し「観測した矛盾事実のみ」を明記
- ★`grok-2-critic.md` §4,5,6★: 「感情核ワード(構成 Gem 用素材)」→「感情核ワード」、「関連トピック(構成 Gem への申し送り)」→「共有体験トピック」、★口調提案・角度提案を全廃★
- ★`grok-3-trend.md`★: 「**想定 id:1 戦略**: A/B/C/D 提案」「**想定動画タイプ**(縦/横)」「**角度の提案**」を ★全削除★。残るのは「数字の矛盾候補(事実として観測)」のみ
- ★`grok-project-instructions.md`★: 旧 2 Gem 言及(「構成 Gem(ショート版)」「JSON Gem」)を全削除、単一 Gem 設計に統一。新セクション「★Gem の専管事項に踏み込まない★」を追加(Grok が触れない領域 vs Grok の領域を明文化)

**Knowledge 側の残骸除去:**
- ★`composition-rules.md`★: チェックリスト項目「□ Gem の申し送り欄あり」削除
- ★`json-schema-rules.md`★: 「★例外★: id:1 戦略(A/B/C/D)が申し送りに指定されている場合〜」→「id:1 戦略を Gem が選択した場合〜」に修正、id:1 戦略の選択は Gem の専管事項(Grok リサーチでは触れない)と明記

**アプリ側コメント修正:**
- `JsonPanel.jsx` 内の「JSON Gem の Knowledge Files」「JSON Gem データ単体モード」を「Gem の」に統一

**最終結果:**
- ✅ 全 Knowledge ファイルから「構成 Gem」「JSON Gem」「申し送り」言及ゼロ
- ✅ 全 Grok エージェントから「申し送り」「id:1 戦略提案」「角度提案」「口調提案」ゼロ
- ✅ Grok の出力は ★事実とファン心理の観測のみ★、Gem は受け取った素材で全構成判断

**NotebookLM 再アップロード推奨ファイル(中身変更):**
- `composition-rules.md`
- `json-schema-rules.md`

**Grok カスタムエージェント再設定推奨ファイル(中身変更):**
- `grok-v2/grok-agent-master.md`
- `grok-v2/grok-1-researcher.md`
- `grok-v2/grok-2-critic.md`
- `grok-v2/grok-3-trend.md`
- `grok-v2/grok-project-instructions.md`
